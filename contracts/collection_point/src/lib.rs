...
impl CollectionPoint {
    ...
    pub fn register_point(
        ...
        // Validate inputs
        common::validation::validate_collection_point_name(&name).expect("Invalid name");
        common::validation::validate_location(&location).expect("Invalid location");
        ...
        // Emit event
        common::CollectionPointEvents::registered(&env, point_id, owner.clone());
    ...
    pub fn get_point(env: Env, point_id: u64) -> common::CollectionPoint {
        read_point(&env, point_id).expect("Collection point not found")
    }
    ...
    pub fn get_point_by_owner(env: Env, owner: Address) -> u64 {
        read_point_by_owner(&env, &owner).expect("No collection point found for owner")
    }
    ...
    pub fn verify_point(env: Env, point_id: u64) {
        ...
        // Get point
        let mut point_data = read_point(&env, point_id).expect("Collection point not found");
    ...
    pub fn update_verification_status(env: Env, point_id: u64, status: common::VerificationStatus) {
        common::Initializable::require_initialized(&env).expect("Not initialized");
